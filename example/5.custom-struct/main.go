...
func main() {
	...
	// 创建管理者
	manager := gorm.GetStructManager(xmlConfig, model.Test{})
	// 当有多个数据源时可以使用 切换数据源
	manager.Engine.Use(gorm.DefaultClientAlias)
	// manager有四个方法
	// 查询数据集 参数SQL的别名 查询条件
	values, err := manager.Query("GetList", map[string]string{})
	if err == nil {
		// 返回的values是结构体数组 但GO中缺少泛型，所以需要手动转换为对应类型
		// 转换列表
		var list []model.Test
		for i := 0; i < len(values); i++ {
			list = append(list, *values[i].Interface().(*model.Test))
		}
		fmt.Println(list)
	}
	// 查询实体
	value, err := manager.QueryFirst("GetModel", map[string]string{})
	if err == nil {
		// 返回的values是结构体 但GO中缺少泛型，所以需要手动转换为对应类型
		item := *value.Interface().(*model.Test)
		fmt.Println(item)
	}
	// 查询Count
